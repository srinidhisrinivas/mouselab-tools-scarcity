...
from mouselab.policies import FunctionPolicy, LiederPolicy, MaxQPolicy
from mouselab.utils import cum_returns
from mouselab.value_functions import LiederQ
...
def get_q_error(theta, envs, parallel=None):
    ...
    for e in envs:
        ...
        q_hat = np.array(
            [Q.predictOne(s, a) for s, a in zip(tr["states"], tr["actions"])]
        )
    ...
    **kwargs
    ...
    if q_learning:
        ...
        def objective(x):
            ...
            if verbose:
                print(
                    theta.round(3), "->", round(error, 3), "in", round(t.elapsed), "sec"
                )
            return error
        ...
        def objective(x):
            ...
            if verbose:
                print(
                    theta.round(3), "->", round(util, 3), "in", round(t.elapsed), "sec"
                )
            return -util

    bounds = [(1.0, max_cost)] + [(0.0, 1.0)] * 3
    with Timer() as t:
        result = gp_minimize(
            objective, bounds, n_calls=n_calls, random_state=0, **kwargs
        )
    ...
    print("BO:", theta.round(3), "->", round(util, 3), "in", round(t.elapsed), "sec")
    if return_result:
