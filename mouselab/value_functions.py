import numpy as np
from keras.models import Dense, Sequential
from keras.optimizers import LinearSGD, Nadam

from mouselab.agents import Component
...
class ValueFunction(Component):
    ...
    def __init__(self, learn_rate=0.1, discount=1):
        super().__init__()
...
class NeuralQ(ActionValueFunction):
    ...
    def _build_model(self):
        ...
        # actor.summary()
        actor.compile(loss="mse", optimizer=Nadam(self.learn_rate))
        return actor
...
class BayesianRegressionV(StateValueFunction):
    ...
    #     # don't learn about final state
    #     self.model.update(batch['states'][:-1], batch['returns'])
